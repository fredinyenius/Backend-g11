...
def enviar_correo_adjuntos(destinatario, titulo):
    ...
    ruta = getcwd()
    # path.join > en base alos parametros que nosotros le pasemos creara la cadena de direcciones ya sea con '/' '\?
    ruta_definitiva = path.join(ruta, 'utils', 'lapiceros.jpeg')
    print(getcwd())
    with open(ruta_definitiva, 'rb') as archivo:
        print(archivo)
        archivo = MIMEApplication(archivo.read(), Name='lapiceros.jpeg')

    archivo['Content-Deposition'] = 'attachment; filename="lapiceros.jpeg"'
